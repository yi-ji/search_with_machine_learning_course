...
def get_index(node) -> int:
    return len(node.find('categoryPath'))-1

def transform_target(child) -> str:
    return child.find('categoryPath')[get_index(child)][0].text
        ...
        if filename.endswith('.xml'):
            ...
            for child in root:
                if (child.find('name') is not None and child.find('name').text is not None and
                        ...
                        child.find('categoryPath')[get_index(child)][0].text is not None):
                    categories.append(child.find('categoryPath')[get_index(child)][0].text)
